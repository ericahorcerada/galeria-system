...
export default function ArtistsPage() {
  ...
  useEffect(() => {
    async function loadPage() {
      try {
        const [settingsResponse, artistsResponse] = await Promise.all([
          fetch("/api/artists-page", { cache: "no-store" }),
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        <section
          className="relative flex min-h-[430px] items-center justify-center overflow-hidden bg-[#3b1d12] bg-cover bg-center px-6 pb-24 pt-40 text-center text-white"
          style={
            settings.hero_background_url
              ? {
                  backgroundImage: `linear-gradient(rgba(0,0,0,0.58), rgba(0,0,0,0.58)), url(${settings.hero_background_url})`,
                }
              : undefined
          }
        >
          ...
          <h1 className="pointer-events-none absolute left-1/2 top-14 -translate-x-1/2 whitespace-nowrap font-serif text-6xl font-black uppercase tracking-wide text-white/20 sm:text-7xl md:text-8xl lg:text-9xl">
            {settings.hero_title}
          </h1>
          ...
          <div className="relative z-10 mx-auto max-w-5xl">
            ...
            <p className="mx-auto mt-8 max-w-3xl text-lg font-semibold leading-8">
