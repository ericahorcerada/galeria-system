...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    async function checkAuth() {
      try {
        ...
        if (mounted) setIsLoggedIn(Boolean(response.ok && result.user?.role === "customer"));
      } catch {
  ...
  const handleCheckoutSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccessMessage(`${result.message} Order number: ${result.order.orderNumber}. Total: ${formatPeso(result.order.totalAmount)}.`);
    } catch {
                        ...
                        <div key={item.id} className="flex gap-4 rounded-lg border p-4">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                              ...
                              <p className="font-medium text-foreground">{formatPeso(item.price * item.quantity)}</p>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (!isLoggedIn) { router.push("/login?next=/cart"); return; }
                      ...
                      <form onSubmit={handleCheckoutSubmit} className="space-y-4">
                        <div className="space-y-4">
                          ...
                          <div className="grid gap-4 sm:grid-cols-2">
                            <div>
                              ...
                              <Input id="firstName" name="firstName" value={checkoutForm.firstName} onChange={handleInputChange} required />
                            </div>
                            <div>
                              <Label htmlFor="lastName">Last Name</Label>
                              <Input id="lastName" name="lastName" value={checkoutForm.lastName} onChange={handleInputChange} required />
                            </div>
                          </div>
                          <div>
                            <Label htmlFor="email">Email</Label>
                            <Input id="email" name="email" type="email" value={checkoutForm.email} onChange={handleInputChange} required />
                          </div>
                          <div>
                            <Label htmlFor="phone">Phone</Label>
                            <Input id="phone" name="phone" type="tel" value={checkoutForm.phone} onChange={handleInputChange} required />
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <Input id="address" name="address" value={checkoutForm.address} onChange={handleInputChange} required />
                          </div>
                          <div className="grid gap-4 sm:grid-cols-2">
                            <div>
                              <Label htmlFor="city">City</Label>
                              <Input id="city" name="city" value={checkoutForm.city} onChange={handleInputChange} required />
                            </div>
                            <div>
                              <Label htmlFor="postalCode">Postal Code</Label>
                              <Input id="postalCode" name="postalCode" value={checkoutForm.postalCode} onChange={handleInputChange} required />
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Your order is saved in MySQL and inventory is reserved immediately. Online payment instructions can be confirmed by staff.
                          </p>
                        </div>
