...
async function seedData() {
  ...
  try {
    ...
    await connection.query(`
      INSERT INTO artworks (title, artist_name, description, category_id, image_url) VALUES
      ...
      ('Tondo Murals', 'AG Saño', 'Documentary print of the colorful murals found in Tondo, Butuan City.', 8, '/artworks/sano-tondo.jpg'),
    ...
    for (const artwork of artworks) {
      for (const size of sizes) {
        for (const material of materials) {
          ...
          await connection.query(
            `INSERT INTO prints (artwork_id, size, material, price, stock_quantity) VALUES (?, ?, ?, ?, ?)
    ...
    await connection.query(`
      INSERT INTO products (category_id, supplier_id, name, sku, barcode, cost_price, selling_price, stock_qty, reorder_level, unit, status) VALUES
      -- Framing Products
      ...
      -- Printing Services (Represented as products)
    ...
    await connection.query(`
      INSERT INTO discounts (name, type, value, valid_from, valid_until, applies_to, status) VALUES
    ...
    for (let daysAgo = 30; daysAgo >= 0; daysAgo--) {
      ...
      for (let i = 0; i < numSales; i++) {
        const cashierId = Math.floor(Math.random() * 6) + 3; // cashier IDs 3-8
    ...
    for (const sale of salesData) {
      await connection.query(`
        ...
        VALUES (?, ?, ${sale[2]}, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [sale[0], sale[1], sale[3], sale[4], sale[5], sale[6], sale[7], sale[8], sale[9], sale[10]]);
    }
    ...
    for (const sale of sales) {
      const numItems = Math.floor(Math.random() * 4) + 1; // 1-5 items per sale
      let remainingSubtotal = sale.subtotal;
      
      for (let i = 0; i < numItems && remainingSubtotal > 0; i++) {
    ...
    await connection.query(`
      INSERT INTO purchase_orders (supplier_id, cashier_id, order_date, received_date, total_cost, status) VALUES
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const type = movementTypes[Math.floor(Math.random() * movementTypes.length)] as keyof typeof reasons;
    ...
    console.log("==========================================");
    console.log("Database seeded successfully with live data!");
    console.log("==========================================");
    console.log("Summary:");
    console.log("- 10 Categories");
    console.log("- 10 Suppliers (Real PH art suppliers)");
    console.log("- 30 Artworks (Real Filipino artists)");
    console.log("- 750 Prints (variations of artworks)");
    console.log("- 34 Products (frames, supplies, merchandise)");
    console.log("- 20 Customers (Filipino names)");
    console.log("- 8 Cashiers/Staff");
    console.log("- 10 Discounts (Active promotions)");
    console.log("- 300+ Sales (30 days history)");
    console.log("- 800+ Sale Items");
    console.log("- 8 Purchase Orders");
    console.log("- 24+ PO Items");
